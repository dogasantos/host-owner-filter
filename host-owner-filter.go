package main

/*
 TODO:
	- take the list of known hosts (seed), perform a whois to collect known strings (Organizations, Names, Emails)
		- use that as base to match across given domains in order to find correlation
	- GOOGLE UA-ID
	- get the A record and check via WHOIS to match the owner against those words collected in the above step
	- check if the CNAME record points towards a known domain name

*/ 
import (
	"fmt"
	"flag"
	"os"
	"io/ioutil"
	"strings"
	"github.com/projectdiscovery/retryabledns"
	"github.com/miekg/dns"
	"github.com/bobesa/go-domain-util/domainutil"
	whoisparser "github.com/likexian/whois-parser-go"
	"github.com/likexian/whois-go"
	"sync"

)

const banner = `
 _               _                                            __ _ _ _
| |__   ___  ___| |_       _____      ___ __   ___ _ __      / _(_) | |_ ___ _ __
| '_ \ / _ \/ __| __|____ / _ \ \ /\ / / '_ \ / _ \ '__|____| |_| | | __/ _ \ '__|
| | | | (_) \__ \ ||_____| (_) \ V  V /| | | |  __/ | |_____|  _| | | ||  __/ |
|_| |_|\___/|___/\__|     \___/ \_/\_/ |_| |_|\___|_|       |_| |_|_|\__\___|_|
==================================================================================
`
const Version = `0.1`
var RootSoa = []string {
	"ADMIN.DNS2.SERVER.AG",
	"ADMIN.NS2.DNSSERVER.AG",
	"ADMIN.bible.museum",
	"BSADMIN.COB.EDU.bs",
	"BSADMIN.COB.edu.bs",
	"CON1.NIPR.mil",
	"DNS.NIC.bs",
	"DNS2.SERVER.AG",
	"FRCU.EUN.eg",
	"FRCU.eun.eg",
	"FSEC-DNS-00.qidc.ne.jp",
	"NS.eu.org",
	"NS.nsk.ru",
	"NS2.DNSSERVER.AG",
	"Postmaster.FRCU.EUN.eg",
	"Postmaster.FRCU.eun.eg",
	"a-dns.pl",
	"a.au",
	"a.bh.centralnic-dns.com",
	"a.cctld.us",
	"a.dns-servers.net.ru",
	"a.dns.br",
	"a.dns.cn",
	"a.dns.flexireg.ru",
	"a.dns.gandi.net",
	"a.dns.nic.aco",
	"a.dns.nic.gea",
	"a.dns.py",
	"a.dns.ripn.net",
	"a.dns.tw",
	"a.dnspod.com",
	"a.edu-servers.net",
	"a.fi",
	"a.gmoregistry.net",
	"a.gov-servers.net",
	"a.gov.lv",
	"a.gtld-servers.net",
	"a.gtld.biz",
	"a.gtld.travel",
	"a.hu",
	"a.nic.afl",
	"a.nic.bank",
	"a.nic.barclaycard",
	"a.nic.barclays",
	"a.nic.bg",
	"a.nic.brussels",
	"a.nic.cancerresearch",
	"a.nic.cba",
	"a.nic.ch",
	"a.nic.cl",
	"a.nic.cloud",
	"a.nic.commbank",
	"a.nic.coop",
	"a.nic.courses",
	"a.nic.cuisinella",
	"a.nic.design",
	"a.nic.et",
	"a.nic.film",
	"a.nic.fr",
	"a.nic.giving",
	"a.nic.ibm",
	"a.nic.insurance",
	"a.nic.krd",
	"a.nic.kw",
	"a.nic.latrobe",
	"a.nic.li",
	"a.nic.lv",
	"a.nic.melbourne",
	"a.nic.men",
	"a.nic.menu",
	"a.nic.monash",
	"a.nic.mtn",
	"a.nic.my",
	"a.nic.net.mm",
	"a.nic.netbank",
	"a.nic.olayan",
	"a.nic.olayangroup",
	"a.nic.one",
	"a.nic.philips",
	"a.nic.physio",
	"a.nic.rs",
	"a.nic.sandvik",
	"a.nic.sandvikcoromant",
	"a.nic.saxo",
	"a.nic.scb",
	"a.nic.schmidt",
	"a.nic.seek",
	"a.nic.study",
	"a.nic.sucks",
	"a.nic.sydney",
	"a.nic.tab",
	"a.nic.tm",
	"a.nic.trust",
	"a.nic.vlaanderen",
	"a.nic.voting",
	"a.nic.walter",
	"a.nic.wtc",
	"a.ns.cf",
	"a.ns.dns.be",
	"a.ns.ga",
	"a.ns.gq",
	"a.ns.ml",
	"a.ns.mt",
	"a.ns.nic.cz",
	"a.ns.nic.shia",
	"a.ns.rs.ba",
	"a.ns.tk",
	"a.nuuk.nic.gl",
	"a.riu.edu.ar",
	"a.root-servers.net",
	"a.thains.co.th",
	"a.tld.lt",
	"a.twnic.net.tw",
	"a.udrtld.net",
	"a.zdnscloud.com",
	"a0.aero.afilias-nst.info",
	"a0.asia.afilias-nst.info",
	"a0.bm.afilias-nst.info",
	"a0.cctld.afilias-nst.info",
	"a0.info.afilias-nst.info",
	"a0.mobi.afilias-nst.info",
	"a0.nic.abbott",
	"a0.nic.ac",
	"a0.nic.agakhan",
	"a0.nic.akdn",
	"a0.nic.alibaba",
	"a0.nic.alipay",
	"a0.nic.ally",
	"a0.nic.apple",
	"a0.nic.archi",
	"a0.nic.audi",
	"a0.nic.autos",
	"a0.nic.avianca",
	"a0.nic.bcg",
	"a0.nic.beats",
	"a0.nic.bet",
	"a0.nic.bio",
	"a0.nic.black",
	"a0.nic.blue",
	"a0.nic.bnpparibas",
	"a0.nic.boats",
	"a0.nic.cern",
	"a0.nic.cipriani",
	"a0.nic.creditunion",
	"a0.nic.dabur",
	"a0.nic.delta",
	"a0.nic.dot",
	"a0.nic.dtv",
	"a0.nic.dunlop",
	"a0.nic.edeka",
	"a0.nic.emerck",
	"a0.nic.extraspace",
	"a0.nic.fage",
	"a0.nic.frogans",
	"a0.nic.gallup",
	"a0.nic.global",
	"a0.nic.goodyear",
	"a0.nic.green",
	"a0.nic.hdfcbank",
	"a0.nic.helsinki",
	"a0.nic.hermes",
	"a0.nic.hkt",
	"a0.nic.homedepot",
	"a0.nic.homes",
	"a0.nic.icbc",
	"a0.nic.io",
	"a0.nic.ist",
	"a0.nic.istanbul",
	"a0.nic.jio",
	"a0.nic.jll",
	"a0.nic.kim",
	"a0.nic.lamborghini",
	"a0.nic.lasalle",
	"a0.nic.lds",
	"a0.nic.lgbt",
	"a0.nic.locker",
	"a0.nic.lotto",
	"a0.nic.ltda",
	"a0.nic.marriott",
	"a0.nic.mit",
	"a0.nic.mormon",
	"a0.nic.motorcycles",
	"a0.nic.ngo",
	"a0.nic.nokia",
	"a0.nic.nowtv",
	"a0.nic.nra",
	"a0.nic.ollo",
	"a0.nic.ong",
	"a0.nic.onl",
	"a0.nic.organic",
	"a0.nic.ott",
	"a0.nic.pccw",
	"a0.nic.pet",
	"a0.nic.pink",
	"a0.nic.pnc",
	"a0.nic.poker",
	"a0.nic.promo",
	"a0.nic.red",
	"a0.nic.redstone",
	"a0.nic.redumbrella",
	"a0.nic.reliance",
	"a0.nic.rich",
	"a0.nic.richardli",
	"a0.nic.ril",
	"a0.nic.sbi",
	"a0.nic.scholarships",
	"a0.nic.sew",
	"a0.nic.sh",
	"a0.nic.shaw",
	"a0.nic.shiksha",
	"a0.nic.sina",
	"a0.nic.ski",
	"a0.nic.srl",
	"a0.nic.stada",
	"a0.nic.star",
	"a0.nic.statebank",
	"a0.nic.stockholm",
	"a0.nic.temasek",
	"a0.nic.thd",
	"a0.nic.travelers",
	"a0.nic.travelersinsurance",
	"a0.nic.trv",
	"a0.nic.tvs",
	"a0.nic.ups",
	"a0.nic.vegas",
	"a0.nic.vig",
	"a0.nic.viking",
	"a0.nic.volkswagen",
	"a0.nic.vote",
	"a0.nic.voto",
	"a0.nic.watches",
	"a0.nic.weibo",
	"a0.nic.xin",
	"a0.nic.yachts",
	"a0.nic.zara",
	"a0.org.afilias-nst.info",
	"a0.post.afilias-nst.info",
	"a0.pr.afilias-nst.info",
	"a0.pro.afilias-nst.info",
	"a1.domain.by",
	"a5.nstld.com",
	"abuse.dns.py",
	"abuse.education.ky.gov",
	"abuse.moe.edu.tw",
	"abuse.opticaljungle.com",
	"ac-at-master.aco.net",
	"ac1.nstld.com",
	"ada.gouv.ml",
	"adam.neoip.com",
	"adm.infotel.kg",
	"admin-c.iga.gov.bh",
	"admin-c.oac.gov.by",
	"admin-dns.cira.ca",
	"admin-dns.registry.co.ca",
	"admin.adie.sn",
	"admin.gov.mg",
	"admin.mil.lv",
	"admin.mil.ru",
	"admin.nic.gm",
	"admin.nic.gov.ua",
	"admin.nic.kw",
	"admin.nic.tm",
	"admin.nic.tt",
	"admin.o7.com",
	"admin.registry.gov.mm",
	"admin.registry.gy",
	"admin.saratov.ru",
	"admin.tm.za",
	"admin.tsk.ru",
	"admin.validname.com",
	"admin.vlc.ru",
	"admin.wask.wroc.pl",
	"administrator.gov.rs",
	"admins.belgorod.ru",
	"admins.gov.ky",
	"adns0.wiscnet.net",
	"aduans.alaska.edu",
	"alejandrareynoso.cctld.gt",
	"alma978b.yahoo.com",
	"amanda.ns.cloudflare.com",
	"amir.leftor.com",
	"andrey.telecom.tm",
	"ans1.aso.ee",
	"ans1.sis.sd",
	"any-ns1.nc",
	"anycast9.irondns.net",
	"anyns.uem.mz",
	"ao01.dns.pt",
	"apple.wa-k20.net",
	"argus.more.net",
	"arizona.edu",
	"as.asiainfo.kg",
	"ashar.pknic.net.pk",
	"asnsvr.asc.edu",
	"astrakhan.ru",
	"ate-ns01.ate.info",
	"awsdns-hostmaster.amazon.com",
	"azuredns-hostmaster.microsoft.com",
	"b.dns.id",
	"b.dns.si",
	"b.in-addr-servers.arpa",
	"b.ip6-servers.arpa",
	"b.nic.dk",
	"b.nic.my",
	"b.ns.ie",
	"babimbi.gov.cm",
	"bds-tlcm-ns-001.gov.bb.co.bb",
	"be-hostmaster.dnsbelgium.be",
	"beau.ns.cloudflare.com",
	"ben.ns.cloudflare.com",
	"bfcsp.belgorod.su",
	"bindmaster.mari-el.ru",
	"bindmaster.mv.ru",
	"bindmaster.relinfo.ru",
	"bindmaster.syzran.ru",
	"blox.net.umd.edu",
	"bosna.utic.net.ba",
	"bow1.intnet.dj",
	"brussels-hostmaster.dnsbelgium.be",
	"bsadmin.cob.edu.bs",
	"c.ci-servers.org",
	"c.dns.ar",
	"c.ns.bcims.net",
	"c.tld.ma",
	"c1.dns.sa",
	"catcher-in-the-rye.nic.nu",
	"catcher-in-the-rye.nic.se",
	"cctldga-tech.nic.ga",
	"cen.po.state.ct.us",
	"chambo.sdnp.org.mw",
	"charm.norid.no",
	"clint.ns.cloudflare.com",
	"cloud-dns-hostmaster.google.com",
	"coby.ns.cloudflare.com",
	"coco.ns.cloudflare.com",
	"com.mo",
	"contact-tech.dnsafrica.net",
	"contact.defense.tn",
	"cu-tech.ceniai.inf.cu",
	"cu-tech.ceniai.net.cu",
	"cudess1.umt.edu",
	"curiosity.dns.pt",
	"cwserv.vpn.gov.ie",
	"cydns.ucy.ac.cy",
	"cynic.dns.cy",
	"d.nic.so",
	"ddimaster.bor.usg.edu",
	"delta.hostmaster.net.ua",
	"delta.hostmaster.ua",
	"demand.alpha.aridns.net.au",
	"di-cox.ec.europa.eu",
	"digicelbusinesssvg.digicelgroup.com",
	"dmaster.ns.jm",
	"dna.sgnic.sg",
	"dns-adm.seciu.edu.uy",
	"dns-admin.antel.net.uy",
	"dns-admin.google.com",
	"dns-admin.greatbasin.net",
	"dns-admin.omadhina.net",
	"dns-admin.posix.co.za",
	"dns-admin.ural.rt.ru",
	"dns-b.iij.ad.jp",
	"dns-external-master.amazon.com",
	"dns-external-master.cn-north-1.amazonaws.cn",
	"dns-fr.dnsafrica.net",
	"dns-hosting.yandex.ru",
	"dns-managers.iij.ad.jp",
	"dns-master.magadan.ru",
	"dns-master.vnnic.vn",
	"dns-operation.switch.ch",
	"dns-sgm.ohio.gov",
	"dns-support.idc.jp",
	"dns-team.flexireg.net",
	"dns-updates.school.za",
	"dns.aeda.ae",
	"dns.amnic.net",
	"dns.amu.edu.pl",
	"dns.audns.net.au",
	"dns.bd",
	"dns.ceger.gov.pt",
	"dns.cloudflare.com",
	"dns.constellix.com",
	"dns.dict.gov.ph",
	"dns.dnsmadeeasy.com",
	"dns.edu.cn",
	"dns.edu.it",
	"dns.esu10.org",
	"dns.fastdns24.com",
	"dns.intnet.dj",
	"dns.jomax.net",
	"dns.lttnet.net",
	"dns.lvrtc.lv",
	"dns.magwien.gv.at",
	"dns.mauritania.mr",
	"dns.nic.at",
	"dns.nic.bs",
	"dns.nic.cr",
	"dns.nic.it",
	"dns.nic.lv",
	"dns.nic.ru",
	"dns.nitc.gov.jo",
	"dns.nl.eu.org",
	"dns.omniway.sm",
	"dns.onenet.net",
	"dns.ph.net",
	"dns.polsl.pl",
	"dns.register.si",
	"dns.ripe.net",
	"dns.sispa.org.sz",
	"dns.task.gda.pl",
	"dns.telekom.net.sb",
	"dns.tld.sy",
	"dns.uen.net",
	"dns.unitech.ac.pg",
	"dns.uts.cw",
	"dns.widhost.net",
	"dns0.mts.ru",
	"dns01.city.nagoya.jp",
	"dns01.gouv.ml",
	"dns1.doc.state.ne.us",
	"dns1.easydns.com",
	"dns1.emdns.uk",
	"dns1.fsu.edu",
	"dns1.fweds-spc.it",
	"dns1.gov.bm",
	"dns1.k12.or.us",
	"dns1.merit.net",
	"dns1.namebay.com",
	"dns1.nic.abogado",
	"dns1.nic.audible",
	"dns1.nic.author",
	"dns1.nic.aws",
	"dns1.nic.bayern",
	"dns1.nic.bbc",
	"dns1.nic.beer",
	"dns1.nic.bentley",
	"dns1.nic.bot",
	"dns1.nic.broadway",
	"dns1.nic.budapest",
	"dns1.nic.buy",
	"dns1.nic.call",
	"dns1.nic.casa",
	"dns1.nic.circle",
	"dns1.nic.cooking",
	"dns1.nic.cymru",
	"dns1.nic.dds",
	"dns1.nic.deal",
	"dns1.nic.fashion",
	"dns1.nic.fast",
	"dns1.nic.fire",
	"dns1.nic.fishing",
	"dns1.nic.fit",
	"dns1.nic.garden",
	"dns1.nic.gop",
	"dns1.nic.got",
	"dns1.nic.horse",
	"dns1.nic.imdb",
	"dns1.nic.jot",
	"dns1.nic.joy",
	"dns1.nic.kindle",
	"dns1.nic.km",
	"dns1.nic.law",
	"dns1.nic.like",
	"dns1.nic.locus",
	"dns1.nic.london",
	"dns1.nic.luxe",
	"dns1.nic.miami",
	"dns1.nic.moi",
	"dns1.nic.net.bw",
	"dns1.nic.now",
	"dns1.nic.pin",
	"dns1.nic.prime",
	"dns1.nic.read",
	"dns1.nic.rodeo",
	"dns1.nic.room",
	"dns1.nic.safe",
	"dns1.nic.save",
	"dns1.nic.silk",
	"dns1.nic.smile",
	"dns1.nic.spot",
	"dns1.nic.surf",
	"dns1.nic.talk",
	"dns1.nic.tunes",
	"dns1.nic.tushu",
	"dns1.nic.uk",
	"dns1.nic.vip",
	"dns1.nic.virgin",
	"dns1.nic.vodka",
	"dns1.nic.wales",
	"dns1.nic.wanggou",
	"dns1.nic.wedding",
	"dns1.nic.work",
	"dns1.nic.yamaxun",
	"dns1.nic.yoga",
	"dns1.nic.you",
	"dns1.nic.zappos",
	"dns1.osogrande.com",
	"dns1.p04.nsone.net",
	"dns1.pole-emploi.fr",
	"dns1.psc.edu",
	"dns1.registrar-servers.com",
	"dns1.tbr.edu",
	"dns1.telekom.net.sb",
	"dns1.tld.tutby.com",
	"dns1.yandex.net",
	"dns101.ovh.net",
	"dns17.ovh.net",
	"dns2.alsitech.it",
	"dns3.gov.ps",
	"dns4.tn.gov",
	"dns7.gov.tt",
	"dnsadm.amu.edu.pl",
	"dnsadm.free.net",
	"dnsadmin.advsys.co.uk",
	"dnsadmin.cavehill.uwi.edu",
	"dnsadmin.citrt.ru",
	"dnsadmin.com.mo",
	"dnsadmin.dns.com",
	"dnsadmin.doit.nh.gov",
	"dnsadmin.edu.mo",
	"dnsadmin.gov.za",
	"dnsadmin.mo",
	"dnsadmin.net.mo",
	"dnsadmin.nic.cl",
	"dnsadmin.nic.ec",
	"dnsadmin.org.mo",
	"dnsadmin.sn.apc.org",
	"dnsadmin.sndi.ci",
	"dnsadmin.soc.lib.md.us",
	"dnsadmin.zadna.org.za",
	"dnsc.ad",
	"dnsmanager.tra.gov.om",
	"dnsmaster.channelisles.net",
	"dnsmaster.corenic.org",
	"dnsmaster.fastweb.it",
	"dnsmaster.gldn.net",
	"dnsmaster.hostmaster.ua",
	"dnsmaster.ic.vrn.ru",
	"dnsmaster.infobox.ru",
	"dnsmaster.irondns.net",
	"dnsmaster.kaluga.ru",
	"dnsmaster.knipp.de",
	"dnsmaster.nask.pl",
	"dnsmaster.nic.dz",
	"dnsmaster.ns.buryatia.ru",
	"dnsmaster.sinor.ru",
	"dnsmaster.tee.gr",
	"dnsmaster.vniitf.ru",
	"dnsop.dns.cv",
	"dnssec.gov.pt",
	"doit.icn.us-domain.illinois.net",
	"domain-admin.univie.ac.at",
	"domain-manager.nic.or.kr",
	"domain-master.ccTLD.ua",
	"domain-master.cctld.ua",
	"domain-master.rift.org.ua",
	"domain.akep.al",
	"domain.tojnet.tj",
	"domainadmin.dnspod.com",
	"domainmaster.cac.washington.edu",
	"domainmaster.k12.or.us",
	"domainmaster.wa-k20.net",
	"domains.cybersmart.co.za",
	"domains.registrar.mw",
	"domeni.marnet.mk",
	"domeni.unibl.rs",
	"domreg.merit.edu",
	"domreg.onenet.net",
	"domreg.usp.ac.fj",
	"dont.use.this.email.nom.za",
	"dotcm.antic.cm",
	"dotus.support.neustar",
	"ds.sgnic.sg",
	"e.registry.qa",
	"edu.mo",
	"edudns-m1.edu.tw",
	"egullich.colo.to",
	"epp.ns.od.ua",
	"f.nic.de",
	"f1g1ns1.dnspod.net",
	"fi-domain-tech.ficora.fi",
	"fred.nic.ao",
	"fred.ns.cloudflare.com",
	"freednsadmin.dnspod.com",
	"ftld02.prd.cira.ca",
	"g.dns.kr",
	"gantulga.ndc.gov.mn",
	"geo.ns.kuban.ru",
	"ginadmin.dits.mof.gov.cy",
	"gl-admin.tele.gl",
	"gm.sr.net",
	"gobadmin.cloudcarib.com",
	"gocns-kedc.gc.ca",
	"gomez.ph.net",
	"gorshkovsv.dv.rt.ru",
	"gouvadmin.gouv.bj",
	"gov.to",
	"grdns.ics.forth.gr",
	"green.uog.edu",
	"gridm.icn.state.ia.us",
	"gromit.grondar.org",
	"gsnhyv2",
	"gtld.alpha.aridns.net.au",
	"gtldsupport.aeda.ae",
	"guy.ns.cloudflare.com",
	"gw01.dns.pt",
	"h.xain.yahoo.com",
	"helpdesk.rcub.bg.ac.rs",
	"hidden-primary.nic.alt.za",
	"hmaster-info.ics.forth.gr",
	"ho1.eunic.net.ua",
	"ho1.net.ua",
	"ho1.ns.gov.ua",
	"ho1.ns.kiev.ua",
	"ho1.ns.pl.ua",
	"host-master.vn.ua",
	"hosting.rdb.rw",
	"hostmaster-serv.entelchile.net",
	"hostmaster.FSEC-DNS-00.qidc.ne.jp",
	"hostmaster.LF.net",
	"hostmaster.NS.nsk.ru",
	"hostmaster.absolight.net",
	"hostmaster.ac.me",
	"hostmaster.alcom.aland.fi",
	"hostmaster.alcom.ax",
	"hostmaster.alt.za",
	"hostmaster.amazon.com",
	"hostmaster.amur.dsv.ru",
	"hostmaster.arizona.edu",
	"hostmaster.art.pl",
	"hostmaster.aso.ee",
	"hostmaster.ate.info",
	"hostmaster.ati.tn",
	"hostmaster.basel.museum",
	"hostmaster.bcims.net",
	"hostmaster.beget.com",
	"hostmaster.belnet.be",
	"hostmaster.bosna.utic.net.ba",
	"hostmaster.bv.nl",
	"hostmaster.carnet.hr",
	"hostmaster.cctld.uz",
	"hostmaster.centralnic.net",
	"hostmaster.ch.eu.org",
	"hostmaster.cherkassy.ua",
	"hostmaster.cloudfront.net",
	"hostmaster.cmc.iq",
	"hostmaster.co.com",
	"hostmaster.cocca.com.au",
	"hostmaster.cocca.com.au.gov.cx",
	"hostmaster.coccaregistry.org",
	"hostmaster.concentric.com",
	"hostmaster.crimea.ua",
	"hostmaster.cv.ua",
	"hostmaster.cyf-kr.edu.pl",
	"hostmaster.cyfronet.krakow.pl",
	"hostmaster.dep.no",
	"hostmaster.dhivehinet.net.mv",
	"hostmaster.dns.ao",
	"hostmaster.dns.ws",
	"hostmaster.dns1.state.ar.us",
	"hostmaster.domain-registry.nl",
	"hostmaster.domreg.lt",
	"hostmaster.donuts.co",
	"hostmaster.dp.ua",
	"hostmaster.dr.na",
	"hostmaster.dreamhost.com",
	"hostmaster.drs.ua",
	"hostmaster.druknet.bt",
	"hostmaster.duckdns.org",
	"hostmaster.dyndns.com",
	"hostmaster.dyndns.org",
	"hostmaster.edu.it",
	"hostmaster.edu.me",
	"hostmaster.eenet.ee",
	"hostmaster.eestiinternet.ee",
	"hostmaster.ena.com",
	"hostmaster.eon.no",
	"hostmaster.epp.ns.od.ua",
	"hostmaster.eu.org",
	"hostmaster.eurodns.com",
	"hostmaster.fastly.com",
	"hostmaster.fsu.edu.lib.fl.us",
	"hostmaster.gandi.net",
	"hostmaster.gdns.net",
	"hostmaster.gov.bb",
	"hostmaster.gov.bf",
	"hostmaster.gov.me",
	"hostmaster.heroku.com",
	"hostmaster.hinet.net",
	"hostmaster.hkirc.net.hk",
	"hostmaster.horizon.net.fk",
	"hostmaster.host.kr.ua",
	"hostmaster.hq.dialcom.nkz.ru",
	"hostmaster.iki.fi",
	"hostmaster.in.ua",
	"hostmaster.info.at",
	"hostmaster.informika.ru",
	"hostmaster.innet.yaroslavl.su",
	"hostmaster.interior.gob.cl",
	"hostmaster.interior.gov.cl",
	"hostmaster.inwx.de",
	"hostmaster.irkutsk.ru",
	"hostmaster.isnic.is",
	"hostmaster.isoc.org.il",
	"hostmaster.its.nc.gov",
	"hostmaster.k12.md.us",
	"hostmaster.kenic.or.ke",
	"hostmaster.kherson.ua",
	"hostmaster.km.ua",
	"hostmaster.lemarit.com",
	"hostmaster.lipetsk.ru",
	"hostmaster.lviv.ua",
	"hostmaster.macomnet.net",
	"hostmaster.magwien.gv.at",
	"hostmaster.mark-itt.ru",
	"hostmaster.mediaserv.net.gf",
	"hostmaster.mnidns1.mninet.ms",
	"hostmaster.mnstate.us",
	"hostmaster.mos.com.np",
	"hostmaster.mts.ru",
	"hostmaster.munnari.oz.au",
	"hostmaster.ncren.net",
	"hostmaster.neonova.net",
	"hostmaster.net.edu.cn",
	"hostmaster.netangels.ru",
	"hostmaster.netim.net",
	"hostmaster.netlink.net.mv",
	"hostmaster.neustar.biz",
	"hostmaster.neustar.in",
	"hostmaster.nic.alsace",
	"hostmaster.nic.amsterdam",
	"hostmaster.nic.ao",
	"hostmaster.nic.aquarelle",
	"hostmaster.nic.bostik",
	"hostmaster.nic.bzh",
	"hostmaster.nic.corsica",
	"hostmaster.nic.cz",
	"hostmaster.nic.dm",
	"hostmaster.nic.do",
	"hostmaster.nic.edu.ua",
	"hostmaster.nic.es",
	"hostmaster.nic.fr",
	"hostmaster.nic.gh",
	"hostmaster.nic.ht",
	"hostmaster.nic.hu",
	"hostmaster.nic.it",
	"hostmaster.nic.kn",
	"hostmaster.nic.kz",
	"hostmaster.nic.la",
	"hostmaster.nic.la.com.la",
	"hostmaster.nic.la.edu.la",
	"hostmaster.nic.la.gov.la",
	"hostmaster.nic.la.info.la",
	"hostmaster.nic.la.int.la",
	"hostmaster.nic.la.net.la",
	"hostmaster.nic.la.org.la",
	"hostmaster.nic.lancaster",
	"hostmaster.nic.lat",
	"hostmaster.nic.leclerc",
	"hostmaster.nic.lk",
	"hostmaster.nic.mma",
	"hostmaster.nic.ms",
	"hostmaster.nic.mtr",
	"hostmaster.nic.mu",
	"hostmaster.nic.museum",
	"hostmaster.nic.mx",
	"hostmaster.nic.my",
	"hostmaster.nic.mz",
	"hostmaster.nic.net.ng",
	"hostmaster.nic.net.sa",
	"hostmaster.nic.nu",
	"hostmaster.nic.ovh",
	"hostmaster.nic.paris",
	"hostmaster.nic.priv.at",
	"hostmaster.nic.rs",
	"hostmaster.nic.ru",
	"hostmaster.nic.sn",
	"hostmaster.nic.sncf",
	"hostmaster.nic.so",
	"hostmaster.nic.st",
	"hostmaster.nic.td",
	"hostmaster.nic.tg",
	"hostmaster.nic.tr",
	"hostmaster.nic.uk",
	"hostmaster.nic.ve",
	"hostmaster.nic.vi",
	"hostmaster.nic.za.net",
	"hostmaster.nic.zm",
	"hostmaster.nl.eu.org",
	"hostmaster.noc.gov.lk",
	"hostmaster.noc.net.er",
	"hostmaster.nominet.org.uk",
	"hostmaster.norid.no",
	"hostmaster.ns.kostroma.ru",
	"hostmaster.ns.nic.ci",
	"hostmaster.ns.nic.mg",
	"hostmaster.ns.novosibirsk.ru",
	"hostmaster.ns.od.ua",
	"hostmaster.ns.odessa.ua",
	"hostmaster.ns1.reg.ru",
	"hostmaster.ns1.tas.gov.au",
	"hostmaster.ns2.dns.ws",
	"hostmaster.nsone.net",
	"hostmaster.oar.net",
	"hostmaster.onatel.bf",
	"hostmaster.opt.nc",
	"hostmaster.org.bw",
	"hostmaster.org.ua",
	"hostmaster.osogrande.com",
	"hostmaster.oxyd.fr",
	"hostmaster.pandi.id",
	"hostmaster.parking.reg.ru",
	"hostmaster.psg.com",
	"hostmaster.rds.org.hn",
	"hostmaster.redcom.ru",
	"hostmaster.register.bg",
	"hostmaster.registrar-servers.com",
	"hostmaster.registro.br",
	"hostmaster.registry.hk.com",
	"hostmaster.registry.hm",
	"hostmaster.renater.fr",
	"hostmaster.riks.ee",
	"hostmaster.rim.mr",
	"hostmaster.ripn.net",
	"hostmaster.riu.edu.ar",
	"hostmaster.rotld.ro",
	"hostmaster.rugion.ru",
	"hostmaster.setarnet.aw",
	"hostmaster.sk-nic.sk",
	"hostmaster.spm.gov.cm",
	"hostmaster.sr.net",
	"hostmaster.state.vt.us",
	"hostmaster.tambov.ru",
	"hostmaster.taxi.aero",
	"hostmaster.te.ua",
	"hostmaster.telecomitalia.sm",
	"hostmaster.telone.co.zw",
	"hostmaster.tenet.ac.za",
	"hostmaster.tld-box.at",
	"hostmaster.tomsknet.ru",
	"hostmaster.tonic.to",
	"hostmaster.tsu.tula.ru",
	"hostmaster.tutby.com",
	"hostmaster.tznic.or.tz",
	"hostmaster.udr.hk.com",
	"hostmaster.uiogh8a3gk-08a73tg8aae.aquila.it",
	"hostmaster.unifish.no",
	"hostmaster.usg.edu",
	"hostmaster.vpn.gov.ie",
	"hostmaster.wiscnet.net",
	"hostmaster.yakutia.ru",
	"hostmaster.zone.eu",
	"hq.dialcom.nkz.ru",
	"huckwong.informac.gov.mo",
	"hzndoitvdns1ext.nhroot.int",
	"ibgb-ext1.gov.bs",
	"ika.maine.edu",
	"ilya.isu.runnet.ru",
	"in1.ns.ua",
	"info.equatorialguineadomains.com",
	"info.erdemnet.mn",
	"info.eunic.net.ua",
	"info.isoc.sd",
	"info.malidili.com",
	"info.nic.ge",
	"info.nic.gw",
	"info.nic.ir",
	"info.ricta.org.rw",
	"info.tomas.uz",
	"info.verisign-grs.com",
	"info.yesnic.com",
	"infoblox-prk-grid01.vfns.net",
	"ins1.c6dns.com",
	"ipadmin.nd.gov",
	"isgate.is",
	"itadmin.osogrande.net",
	"itmanager.fsmtc.fm",
	"its.denic.de",
	"jbroom.nic.gp",
	"jhuwe.mde.k12.ms.us",
	"joan.ns.cloudflare.com",
	"john.vatican.va",
	"joost.zuurbier.dot.tk",
	"josh.ns.cloudflare.com",
	"k-uralsk.ru",
	"k12-ma-us-hostmaster.rsuc.gweep.net",
	"khaleds.lttnet.net",
	"kohilaire.socatel.cf",
	"kontakt.aftermarket.pl",
	"ks355250.kimsufi.com",
	"linli",
	"lito.ibarra.sv",
	"loopback.dns.net.nz",
	"lsadmin.lca.org.ls",
	"lsadmin.nic.ls",
	"lv-o-nm-prod.its.maine.edu",
	"m.mx-ns.mx",
	"m.tld.sk",
	"ma.anrt.ma",
	"maestro.registro.gt",
	"mail.tuva.ru",
	"malawi.mtit.pna.ps",
	"mark.grondar.za",
	"marr.ab.ru",
	"maslamani.mtit.pna.ps",
	"master-dns.cio.gov.bh",
	"master.dns.md",
	"master.nic.ec",
	"master.state.ar.us",
	"mick.kirov.ru",
	"mnidns1.mninet.ms",
	"mo",
	"mosthamster.nic.priv.at",
	"mrfdnsmaster",
	"ms1.dyntld.net",
	"msnhst.microsoft.com",
	"munnari.oz.au",
	"mzizi.kenic.or.ke",
	"n3ns.nic.es",
	"na-ns.anycast.pch.net",
	"nabil.aub.edu.lb",
	"nahouri.onatel.bf",
	"named-mgr.wannms.state.tn.us",
	"ncc.gov.ru",
	"net-admin.city.kobe.jp",
	"net.mo",
	"network.champ.aero",
	"ngo.za",
	"nic.nic.sn",
	"nic.nic.state.mn.us",
	"nic.sta.ad",
	"nic.unh.edu",
	"nicnet.nic.in",
	"nix.ns.ua",
	"nl.dns.eu",
	"nms.cyf-kr.edu.pl",
	"nms.cyfronet.krakow.pl",
	"no-updates.control.vweb.co.za",
	"noc.1gb.com.ua",
	"noc.afilias-nst.info",
	"noc.dns.icann.org",
	"noc.gmoregistry.net",
	"noc.icn.state.ia.us",
	"noc.imaster.ru",
	"noc.inar.ru",
	"noc.ineco.ryazan.su",
	"noc.learn.ac.lk",
	"noc.mediaserv.net.mq",
	"noc.nic.gob.ar",
	"noc.nira.org.ng",
	"noc.nnov.ru",
	"noc.real.astrakhan.ru",
	"noc.restena.lu",
	"noc.s-inform.net",
	"noc.sakhalin.ru",
	"noc.sampo.ru",
	"noc.telekom.ru",
	"ns-01.mcnc.org",
	"ns-1009.awsdns-62.net",
	"ns-1065.awsdns-05.org",
	"ns-1129.awsdns-13.org",
	"ns-1230.awsdns-25.org",
	"ns-1235.awsdns-26.org",
	"ns-1249.awsdns-28.org",
	"ns-1264.awsdns-30.org",
	"ns-1335.awsdns-38.org",
	"ns-1377.awsdns-44.org",
	"ns-148.awsdns-18.com",
	"ns-1622.awsdns-10.co.uk",
	"ns-1676.awsdns-17.co.uk",
	"ns-1961.awsdns-53.co.uk",
	"ns-2005.awsdns-58.co.uk",
	"ns-418.awsdns-52.com",
	"ns-426.awsdns-53.com",
	"ns-48.awsdns-06.com",
	"ns-673.awsdns-20.net",
	"ns-729.awsdns-27.net",
	"ns-741.awsdns-cn-46.com",
	"ns-781.awsdns-33.net",
	"ns-831.awsdns-39.net",
	"ns-896.awsdns-48.net",
	"ns-933.awsdns-52.net",
	"ns-951.awsdns-54.net",
	"ns-978.awsdns-58.net",
	"ns-cloud-c1.googledomains.com",
	"ns-for-microsoft.k12.ma.us",
	"ns-pri.nic.am",
	"ns-root-1.scpt-network.net",
	"ns-tld1.charlestonroadregistry.com",
	"ns.LF.net",
	"ns.ac.me",
	"ns.alcom.aland.fi",
	"ns.amursk.ru",
	"ns.anycast.nic.cx",
	"ns.anycast.nic.nf",
	"ns.baikal.ru",
	"ns.belnet.be",
	"ns.buryatia.ru",
	"ns.ceniai.net.cu",
	"ns.citrt.ru",
	"ns.ck.ua",
	"ns.com.mv",
	"ns.coza.net.za",
	"ns.crimea.ua",
	"ns.cv.ua",
	"ns.dns.cv",
	"ns.dp.ua",
	"ns.edu.me",
	"ns.edu.mv",
	"ns.enet.ru",
	"ns.erdemnet.mn",
	"ns.eu.org",
	"ns.gob.cl",
	"ns.gouv.sn",
	"ns.gov.cl",
	"ns.gov.ee",
	"ns.gov.ky",
	"ns.gov.me",
	"ns.gov.mg",
	"ns.gov.mn",
	"ns.gov.mv",
	"ns.gov.ru",
	"ns.greatbasin.net",
	"ns.host.kr.ua",
	"ns.iki.fi",
	"ns.in.ua",
	"ns.inar.ru",
	"ns.informika.ru",
	"ns.innet.yaroslavl.su",
	"ns.intnet.ne",
	"ns.inwx.de",
	"ns.irkutsk.ru",
	"ns.jm",
	"ns.jpberlin.de",
	"ns.kamchatka.ru",
	"ns.kg",
	"ns.khakassia.ru",
	"ns.kht.ru",
	"ns.kirov.ru",
	"ns.km.ua",
	"ns.kostroma.ru",
	"ns.kuban.ru",
	"ns.macomnet.ru",
	"ns.mark-itt.ru",
	"ns.mpautina.ru",
	"ns.mv",
	"ns.net.mv",
	"ns.ni",
	"ns.nic.bi",
	"ns.nic.bo",
	"ns.nic.cat",
	"ns.nic.ci",
	"ns.nic.ge",
	"ns.nic.kz",
	"ns.nic.lk",
	"ns.nic.me",
	"ns.nic.mg",
	"ns.nic.sn",
	"ns.novosibirsk.ru",
	"ns.nta.mh",
	"ns.od.ua",
	"ns.odessa.ua",
	"ns.omsk.ru",
	"ns.org.mv",
	"ns.pa",
	"ns.phx6.nearlyfreespeech.net",
	"ns.pro.mv",
	"ns.rcub.bg.ac.rs",
	"ns.redcom.ru",
	"ns.relinfo.ru",
	"ns.rubtsovsk.ru",
	"ns.ryazan.ru",
	"ns.sakhalin.ru",
	"ns.snc.ru",
	"ns.snz.ru",
	"ns.syzran.ru",
	"ns.tambov.ru",
	"ns.td-net.ru",
	"ns.te.ua",
	"ns.telekom.ru",
	"ns.tl.ru",
	"ns.tld.ee",
	"ns.tomsknet.ru",
	"ns.tsk.ru",
	"ns.tts.magadan.su",
	"ns.tuva.ru",
	"ns.tver.ru",
	"ns.unbi.ba",
	"ns.uniregistry.net",
	"ns.uz",
	"ns.vdonsk.ru",
	"ns.vlc.ru",
	"ns.vrn.ru",
	"ns.yakutia.ru",
	"ns.zone.eu",
	"ns0.cenpac.net.nr",
	"ns0.centralnic.net",
	"ns0.ja.net",
	"ns0.lolo.co.za",
	"ns0.nic.co.com",
	"ns0.nic.la",
	"ns0.nnov.ru",
	"ns0.oar.net",
	"ns0.pitcairn.net.pn",
	"ns0.plig.net",
	"ns0.state.vt.us",
	"ns01-server.curinfo.cw",
	"ns01.delaware.gov",
	"ns01.gouv.bj",
	"ns01.gov.cy",
	"ns01.idc.jp",
	"ns01.int.click.na",
	"ns01.k12.ne.us",
	"ns01.sc.pref.saitama.jp",
	"ns01.trademarkarea.com",
	"ns1-08.azure-dns.com",
	"ns1-gp.mediaserv.net",
	"ns1.101domain.com",
	"ns1.1gb.ua",
	"ns1.ac.lk",
	"ns1.aedns.ae",
	"ns1.aland.net",
	"ns1.anteldata.com.uy",
	"ns1.ati.tn",
	"ns1.bahnhof.net",
	"ns1.beget.com",
	"ns1.bnnic.bn",
	"ns1.cctld.co",
	"ns1.cen.ct.gov",
	"ns1.city.kobe.jp",
	"ns1.cmc.iq",
	"ns1.co.ba",
	"ns1.co.pl",
	"ns1.com.kp",
	"ns1.communitydns.net",
	"ns1.concentric.com",
	"ns1.defense.tn",
	"ns1.dns.aw",
	"ns1.dns.com",
	"ns1.dns.lu",
	"ns1.dns.net.kh",
	"ns1.dns.net.za",
	"ns1.dns.nic.aaa",
	"ns1.dns.nic.able",
	"ns1.dns.nic.accountant",
	"ns1.dns.nic.aetna",
	"ns1.dns.nic.aig",
	"ns1.dns.nic.amica",
	"ns1.dns.nic.analytics",
	"ns1.dns.nic.aramco",
	"ns1.dns.nic.axa",
	"ns1.dns.nic.bible",
	"ns1.dns.nic.bid",
	"ns1.dns.nic.buzz",
	"ns1.dns.nic.caravan",
	"ns1.dns.nic.cbn",
	"ns1.dns.nic.cbre",
	"ns1.dns.nic.chase",
	"ns1.dns.nic.chintai",
	"ns1.dns.nic.cisco",
	"ns1.dns.nic.club",
	"ns1.dns.nic.coupon",
	"ns1.dns.nic.cricket",
	"ns1.dns.nic.date",
	"ns1.dns.nic.dell",
	"ns1.dns.nic.download",
	"ns1.dns.nic.dupont",
	"ns1.dns.nic.earth",
	"ns1.dns.nic.faith",
	"ns1.dns.nic.ferrero",
	"ns1.dns.nic.flickr",
	"ns1.dns.nic.ford",
	"ns1.dns.nic.frontier",
	"ns1.dns.nic.grainger",
	"ns1.dns.nic.gucci",
	"ns1.dns.nic.health",
	"ns1.dns.nic.hoteles",
	"ns1.dns.nic.hsbc",
	"ns1.dns.nic.ipiranga",
	"ns1.dns.nic.itau",
	"ns1.dns.nic.jmp",
	"ns1.dns.nic.jnj",
	"ns1.dns.nic.jpmorgan",
	"ns1.dns.nic.kinder",
	"ns1.dns.nic.lifeinsurance",
	"ns1.dns.nic.lincoln",
	"ns1.dns.nic.loan",
	"ns1.dns.nic.mlb",
	"ns1.dns.nic.moe",
	"ns1.dns.nic.moto",
	"ns1.dns.nic.mutual",
	"ns1.dns.nic.netflix",
	"ns1.dns.nic.neustar",
	"ns1.dns.nic.northwesternmutual",
	"ns1.dns.nic.nyc",
	"ns1.dns.nic.office",
	"ns1.dns.nic.osaka",
	"ns1.dns.nic.party",
	"ns1.dns.nic.passagens",
	"ns1.dns.nic.pharmacy",
	"ns1.dns.nic.praxi",
	"ns1.dns.nic.qpon",
	"ns1.dns.nic.racing",
	"ns1.dns.nic.review",
	"ns1.dns.nic.rocher",
	"ns1.dns.nic.safety",
	"ns1.dns.nic.sas",
	"ns1.dns.nic.science",
	"ns1.dns.nic.song",
	"ns1.dns.nic.taipei",
	"ns1.dns.nic.taobao",
	"ns1.dns.nic.tdk",
	"ns1.dns.nic.tel",
	"ns1.dns.nic.teva",
	"ns1.dns.nic.tmall",
	"ns1.dns.nic.trade",
	"ns1.dns.nic.tube",
	"ns1.dns.nic.vuelos",
	"ns1.dns.nic.weather",
	"ns1.dns.nic.weatherchannel",
	"ns1.dns.nic.webcam",
	"ns1.dns.nic.williamhill",
	"ns1.dns.nic.win",
	"ns1.dns.nic.zero",
	"ns1.dns.nl",
	"ns1.dns.samsung",
	"ns1.dnsdun.com",
	"ns1.domaindiscount24.net",
	"ns1.dreamhost.com",
	"ns1.druknet.bt",
	"ns1.dyndns.org",
	"ns1.edu.bb",
	"ns1.edu.kp",
	"ns1.edu.tt",
	"ns1.ena.com",
	"ns1.eurodns.com",
	"ns1.evrydns.net",
	"ns1.fastly.net",
	"ns1.free.net",
	"ns1.gldn.net",
	"ns1.gnldm1056.siteground.biz",
	"ns1.google.com",
	"ns1.gov.az",
	"ns1.gov.ge",
	"ns1.gov.kp",
	"ns1.gov.lk",
	"ns1.gov.mo",
	"ns1.gov.ph",
	"ns1.gov.ps",
	"ns1.gov.rs",
	"ns1.gov.st",
	"ns1.gov.za",
	"ns1.horizon.net.fk",
	"ns1.hosting.bhtelecom.ba",
	"ns1.in.gov",
	"ns1.info.at",
	"ns1.infobox.org",
	"ns1.involta.com",
	"ns1.is.lt",
	"ns1.jino.ru",
	"ns1.k12.la.us",
	"ns1.k12.ms.us",
	"ns1.kptc.kp",
	"ns1.ksc.kumamoto.jp",
	"ns1.kyschools.us",
	"ns1.lipetsk.ru",
	"ns1.magic.mn",
	"ns1.mana.pf",
	"ns1.mconnect.aero",
	"ns1.mil.cl",
	"ns1.mil.lv",
	"ns1.mil.ru",
	"ns1.mnstate.us",
	"ns1.mv.ru",
	"ns1.mywebserver.co.za",
	"ns1.nc.gov",
	"ns1.nc.tr",
	"ns1.neoip.com.com.sl",
	"ns1.neoip.com.edu.sl",
	"ns1.neoip.com.gov.sl",
	"ns1.neoip.com.net.sl",
	"ns1.neoip.com.org.sl",
	"ns1.neonova.net",
	"ns1.net.icm.edu.pl",
	"ns1.net24.net.nz",
	"ns1.netangels.ru",
	"ns1.netim.net",
	"ns1.neustar.in",
	"ns1.neustar.vu",
	"ns1.nic.al",
	"ns1.nic.amsterdam",
	"ns1.nic.cm",
	"ns1.nic.do",
	"ns1.nic.dz",
	"ns1.nic.es",
	"ns1.nic.firmdale",
	"ns1.nic.gdn.gdn",
	"ns1.nic.gh",
	"ns1.nic.gm",
	"ns1.nic.gp",
	"ns1.nic.ht",
	"ns1.nic.ir",
	"ns1.nic.ls",
	"ns1.nic.mc",
	"ns1.nic.mp",
	"ns1.nic.mtr",
	"ns1.nic.tg",
	"ns1.nic.tj",
	"ns1.nic.tr",
	"ns1.oac.gov.by",
	"ns1.org.kp",
	"ns1.osogrande.net",
	"ns1.oxyd.net",
	"ns1.p09.dynect.net",
	"ns1.p19.dynect.net",
	"ns1.p23.dynect.net",
	"ns1.pananames.com",
	"ns1.parkingcrew.net",
	"ns1.perf1.com",
	"ns1.pnina.ps",
	"ns1.rdb.rw",
	"ns1.reg.ru",
	"ns1.reg.uz",
	"ns1.registry-servers.4u.com",
	"ns1.registry.hm",
	"ns1.registry.in",
	"ns1.registry.om",
	"ns1.registry.qa",
	"ns1.registy.qa",
	"ns1.renater.fr",
	"ns1.rep.kp",
	"ns1.rkursk.ru",
	"ns1.rspd.tj",
	"ns1.rugion.ru",
	"ns1.ryce-rsp.com",
	"ns1.s-inform.net",
	"ns1.sampo.ru",
	"ns1.sd.gov",
	"ns1.selectel.org",
	"ns1.shop.reg.ru",
	"ns1.silweb.pl",
	"ns1.sispa.org.sz",
	"ns1.smallregistry.net",
	"ns1.sndi.ci",
	"ns1.snt.ru",
	"ns1.state.mn.us",
	"ns1.stavropol.ru",
	"ns1.tas.gov.au",
	"ns1.task.gda.pl",
	"ns1.tee.gr",
	"ns1.teledata.mz",
	"ns1.teleinfo.cn",
	"ns1.tld.sy",
	"ns1.tomsk.ru",
	"ns1.tutby.com",
	"ns1.u.dep.no",
	"ns1.uadns.com",
	"ns1.unifish.no",
	"ns1.unitech.ac.pg",
	"ns1.ural.rt.ru",
	"ns1.vn.ua",
	"ns1.vologda.ru",
	"ns1.washingtondc.gov",
	"ns10.dnsmadeeasy.com",
	"ns11.champ.aero",
	"ns11.constellix.com",
	"ns12.ivanovo.ru",
	"ns1390.ztomy.com",
	"ns1lux.europa.eu",
	"ns2.amur.ru",
	"ns2.dns.ws",
	"ns2.eenet.ee",
	"ns2.nic.dm",
	"ns2.nic.mr",
	"ns2.registry.om",
	"ns2.telone.co.zw",
	"ns2.time.museum",
	"ns2.tznic.or.tz",
	"ns21.cloudns.net",
	"ns3-com.nic.ru",
	"ns3-fwl2.nic.ru",
	"ns3-geo.nic.ru",
	"ns3-l2.nic.ru",
	"ns3.domaindiscount24.net",
	"ns3.hkirc.net.hk",
	"ns3.lowvoice.nl",
	"ns3.mari-el.ru",
	"ns3.registre.ma",
	"ns3.state.nd.us",
	"ns31.cloudns.net",
	"ns4.duckdns.org",
	"ns4.kaluga.ru",
	"ns4.nic.ve",
	"ns42.nic.tr",
	"ns5.dnsmadeeasy.com",
	"ns5.o7.com",
	"ns5.orenburg.ru",
	"ns6.tsu.tula.ru",
	"nsa.lib.md.us",
	"nsa.nic.net.ng",
	"nsa.ns.il",
	"nsa.tld.md",
	"nsadmin.nic.in",
	"nsmaster.nic.fr",
	"nsmaster.prive.nic.sn",
	"nstld.iana.org",
	"nstld.verisign-grs.com",
	"ochre.school.za",
	"ocssc.ohio.gov",
	"ons1.sbcjc.cc.ms.us",
	"operador.rcp.pe",
	"operations.ja.net",
	"operator.ivanovo.ru",
	"ops.teleinfo.cn",
	"ops.uniregistry.net",
	"org.mo",
	"oubri.gov.bf",
	"panel.govhost.st",
	"parau.oyster.net.ck",
	"paulos.sdnp.org.mw",
	"pch.nic.bj",
	"pch.nic.ki",
	"pch.nic.net.bw",
	"pch.nic.td",
	"pch.nic.tl",
	"pch.nic.tt",
	"pch.nic.vi",
	"pch.nic.zm",
	"pch.whois.ai",
	"pdns07.domaincontrol.com",
	"pdns1.ultradns.net",
	"pdns196.ultradns.com",
	"pduggan.asc.edu",
	"pebbles.iom.com",
	"pelican.state.lib.la.us",
	"petra.nic.gov.jo",
	"petra.nitc.gov.jo",
	"ph-tld-ns.dot.ph",
	"phduggan.asnmail.asc.edu",
	"pknic.pk",
	"please_set_email.absolutely.nowhere",
	"polux.entelchile.net",
	"popmi1d03.mfn2.myflorida.com",
	"poptl2d03.mfn2.myflorida.com",
	"postmaster.alaska.edu",
	"postmaster.azed.gov",
	"postmaster.bnnic.bn",
	"postmaster.email.state.az.us",
	"postmaster.ethionet.et",
	"postmaster.gov.bm",
	"postmaster.gov.st",
	"postmaster.insun.azerbaijan.su",
	"postmaster.maritime.museum",
	"postmaster.marr.ab.ru",
	"postmaster.nic.firmdale",
	"postmaster.nic.gdn",
	"postmaster.pole-emploi.fr",
	"postmaster.police.uk",
	"postmaster.rubtsovsk.ru",
	"postmaster.sendtech.net",
	"postmaster.stavropol.ru",
	"postmaster.tbr.edu",
	"postmaster.teledata.mz",
	"postmaster.trentinosudtirol.it",
	"postmaster.umt.umt.edu",
	"postmaster.vatican.va",
	"postmaster.vnnic.vn",
	"prd1.azuredns-cloud.net",
	"prdmlspub02.cira.ca",
	"prdpublish04.cira.ca",
	"pri.authdns.ripe.net",
	"primary.rotld.ro",
	"primary1.aq.dns.net.nz",
	"pscnet-admin.psc.edu",
	"publicdns.delaware.gov",
	"q.au",
	"quipu.rcp.net.pe",
	"ramboa.gmail.com",
	"ramboa.nic.mg",
	"register.more.net",
	"registrar.carnet.hr",
	"registrations.pitcairn.gov.pn",
	"registry-default.nic.se",
	"registry.gc.ca",
	"registry.nic.bi",
	"registry.nic.mr",
	"registry.nic.net.bw",
	"registry.ricta.org.rw",
	"registry.thains.co.th",
	"regops.uniregistry.link",
	"relay.co.ca",
	"request.dns.pt",
	"ressources-numeriques.scpt-network.net",
	"rhcloud-security.redhat.com",
	"rip.psg.com",
	"root.amazon.com",
	"root.city.nagoya.jp",
	"root.cnnic.cn",
	"root.com.kp",
	"root.dns.bd",
	"root.dns.jp",
	"root.dns1.nic.km",
	"root.dns5.gov.tt",
	"root.eahd.or.ug",
	"root.edu.kp",
	"root.enet.ru",
	"root.extride.ad.jp",
	"root.gnldm1056.siteground.biz",
	"root.gov.kp",
	"root.jpberlin.de",
	"root.k-uralsk.ru",
	"root.khakassia.ru",
	"root.kht.ru",
	"root.km",
	"root.kptc.kp",
	"root.mail.kamchatka.ru",
	"root.nic.jprs",
	"root.nic.mc",
	"root.nic.ntt",
	"root.nic.sakura",
	"root.ns.intnet.ne",
	"root.ns.ni",
	"root.ns.pa",
	"root.ns.tl.ru",
	"root.ns.tver.ru",
	"root.ns1.gov.az",
	"root.ns1.is.lt",
	"root.ns1.vologda.ru",
	"root.ns5.orenburg.ru",
	"root.org.kp",
	"root.panel.govhost.st",
	"root.panel.webhosting.st",
	"root.rds.org.hn",
	"root.rep.kp",
	"root.rkursk.ru",
	"root.samara.ru",
	"root.sc.pref.saitama.jp",
	"root.snc.ru",
	"root.state.in.us",
	"root.technodesign.ru",
	"root.websun-com.ru",
	"root.y.net.ye",
	"root3.zone.tc",
	"sah1.ye",
	"samara",
	"sawanew.noc.net.er",
	"seciu.edu.uy",
	"send.mail.to.tech.at.domains.qa",
	"sendmail.tech.at.domains.qa",
	"sendmail.to.tech.at.domains.qa",
	"serg.vdonsk.ru",
	"server.i3c.co.ug",
	"service.desk.nebraska.gov",
	"setup.mil.cl",
	"shikhar.mos.com.np",
	"sir.red.sv",
	"slava.amursk.ru",
	"sns.dns.icann.org",
	"snw.twnic.net.tw",
	"soa.coccaregistry.org",
	"soa.net24.net.nz",
	"soa.nic.kn",
	"soa.nzrs.net.nz",
	"soa.oyster.net.ck",
	"soa.registry.gy",
	"soporte.nic.bo",
	"support.ariservices.com",
	"support.cloudns.net",
	"support.dns.net.za",
	"support.fastvps.ru",
	"support.get.mp",
	"support.involta.com",
	"support.kurgan.ru",
	"support.nearlyfreespeech.net",
	"support.nic.cd",
	"support.nic.coop",
	"support.nic.ru",
	"support.nta.mh",
	"support.registry.net.za",
	"support.ryce-rsp.com",
	"support.test.tj",
	"support.tutby.com",
	"sys.gov.ps",
	"sys.pnina.ps",
	"sysadmin.domains.ph",
	"sysadmin.state.lib.la.us",
	"system.mana.pf",
	"td_dns_gtld.knet.cn",
	"tech.datacom.mn",
	"tech.dk-hostmaster.dk",
	"tech.eurid.eu",
	"tech.key-systems.net",
	"tech.nic.bj",
	"tech.nic.org.mt",
	"tech.ovh.net",
	"tech.webns2.telecom.tm",
	"technical.nameshield.net",
	"technician.cenpac.net.nr",
	"teri.usp.ac.fj",
	"theKdomains.gmail.com",
	"this-is-probably-not-the-email-address-you-are-looking-for.co.za",
	"ti.nic.cr",
	"tld-tech.weare.ie",
	"tld.gdns.net",
	"tld1.marnet.mk",
	"tld1.nic.jprs",
	"tld1.nic.ntt",
	"tld1.nic.sakura",
	"to",
	"tzh.unh.edu",
	"udns1.tld.mu",
	"uognoc.triton.uog.edu",
	"us-domain.illinois.net",
	"us.illinois.net",
	"vince.offshore.ai",
	"vlaanderen-hostmaster.dnsbelgium.be",
	"wask.wask.wroc.pl",
	"web1.telecom.tm",
	"webadministrator.dc.gov",
	"webhosting.com.ba",
	"webhosting.st",
	"webmaster.alsitech.it.valdaosta.it",
	"webmaster.c.la",
	"webmaster.comorestelecom.km",
	"webmaster.state.sd.us",
	"webmaster.tomsk.ru",
	"webns2.telecom.tm",
	"websun-com.ru",
	"www.camnet.com.kh",
	"z.dns.jp",
	"z.dns.nic.berlin",
	"z.dns.nic.hamburg",
	"z.dns.nic.versicherung",
	"zeina.aub.edu.lb",
	"zeta.uar.net",
	"zone-admin.dyndns.com",
	"zone-ds.dns.business",
	"zone-ds.registry.net.za",
	"zone.easydns.com",
	"zone.tc",
	"zoneadmin.cloud.gov.ge"}


var DefaultResolvers = []string{
	"1.1.1.1:53", // Cloudflare
	"1.0.0.1:53", // Cloudflare
	"8.8.8.8:53", // Google
	"8.8.4.4:53", // Google
	"9.9.9.9:53", // Quad9
}

var RootServers = []string {
	"gtld-servers.net",
	"verisign-grs.com",
	"afilias-nst.info",
	"nominet.org.uk",
	"nic.uk",

}
type Options struct {
	Domain         	  string
	Domainf           string
	Domains           []string
	Hosts             string
	Host              string
	WhoisString		  string
	OutputFile        string
	Silent            bool
	Verbose           bool
	//A                 bool
	//CNAME             bool
	//SOA               bool
}

type DomainTokens struct {
	Subdomain string
	Domain string
	Tld string
}


func showBanner() {
	fmt.Printf("%s", banner)
	fmt.Printf("\t\t\t\t\t\t\t\tversion: %s\n\n",Version)
}
func sliceContainsElement(slice []string, element string) bool {
	retval := false
	for _, e := range slice {
		if e == element {
			retval = true
		}
	}
	return retval
}
func sliceUniqueElements(slice []string) []string {
    keys := make(map[string]bool)
    list := []string{}
    for _, entry := range slice {
        if _, value := keys[entry]; !value {
            keys[entry] = true
            if len(entry) > 2 {
                list = append(list, entry)
            }
        }
    }
    return list
}

func (options *Options) validateOptions() {
	if len(options.Hosts) > 0 && len(options.Host) >0 {
		fmt.Println("[x] -l and -u can't be used at the same time.")
		os.Exit(3)
	}

	if len(options.Hosts) == 0 && len(options.Host)  == 0 {
		fmt.Println("[x] Must specify -l or -u")
		os.Exit(3)
	}

	if len(options.Hosts) > 0{
		_, err := os.Stat(options.Hosts)
		if os.IsNotExist(err) {
			fmt.Printf("[x] Input file does not exist: %s",options.Hosts)
			os.Exit(3)
		}
	}
	if options.Verbose && options.Silent {
		fmt.Println("[x] Verbose and Silent can't be used at the same time.")
		os.Exit(3)
	}
	/*if !options.A && !options.CNAME && !options.SOA {
		fmt.Println("[x] Must specify a dns record type to query")
		os.Exit(3)
	}*/
}

func parseOptions() *Options {
	options := &Options{}
	flag.StringVar(&options.Domain, 		"d", "", "Main domain to serve as a seed to compare")
	flag.StringVar(&options.Domainf, 		"D", "", "List of known domains to serve as a seed to compare")
	flag.StringVar(&options.WhoisString, 	"w", "", "Whois string to match (in Org, Email or technicalContact->name")
	flag.StringVar(&options.Hosts, 			"L", "", "File input with list of subdomains")
	flag.StringVar(&options.Host, 			"l", "", "Check a single host")
	flag.StringVar(&options.OutputFile, 	"o", "", "File to write output to (optional)")
	flag.BoolVar(&options.Silent, 			"silent", false, "Show only results in the output")
	flag.BoolVar(&options.Verbose, 			"verbose", false, "Verbose output")
	//flag.BoolVar(&options.A, 				"a", false, "Query A record")
	//flag.BoolVar(&options.CNAME, 			"cname", false, "Query CNAME record")
	//flag.BoolVar(&options.SOA, 				"soa", false, "Query SOA record")
	flag.Parse()

	showBanner()
	options.validateOptions()
	return options
}

func ParseDomainTokens(value string) (*DomainTokens){
	var d DomainTokens
	d.Subdomain = domainutil.Subdomain(value)
	d.Domain = domainutil.Domain(value)
	d.Tld = domainutil.DomainSuffix(value)

	return &d
}

func getParsedWhois(domain string) (result whoisparser.WhoisInfo, err error) {
	whoisRawData, err := whois.Whois(domain)
	if err != nil {
		return
	}
	result, err = whoisparser.Parse(whoisRawData)
	return
}

func whoisCheck(pattern string, domain string) bool {
	var retval = false

	pWhois, err := getParsedWhois(domain)
	if err == nil {
		if pWhois.Registrant != nil {
			//fmt.Printf("Registrant.Name: %s\n",pWhois.Registrant.Name)
			//fmt.Printf("Registrant.Organization: %s\n",pWhois.Registrant.Organization)
			//fmt.Printf("Registrant.Email: %s\n",pWhois.Registrant.Email)	
			if strings.Contains(pWhois.Registrant.Name,pattern) {
				retval = true
			} else if strings.Contains(pWhois.Registrant.Organization,pattern) && retval == false {
				retval = true
			} else if strings.Contains(pWhois.Registrant.Email,pattern) && retval == false {
				retval = true
			}
		}
		if pWhois.Technical != nil && retval == false  {
			//fmt.Printf("Technical.Name: %s\n",pWhois.Technical.Name)
			//fmt.Printf("Technical.Organization: %s\n",pWhois.Technical.Organization)
			//fmt.Printf("Technical.Email: %s\n",pWhois.Technical.Email)

			if strings.Contains(pWhois.Technical.Name,pattern) {
				retval = true
			} else if strings.Contains(pWhois.Technical.Organization,pattern) && retval == false {
				retval = true
			} else if strings.Contains(pWhois.Technical.Email,pattern) && retval == false {
				retval = true
			}
		}

		if pWhois.Administrative != nil && retval == false {
			//fmt.Printf("Administrative.Name: %s\n",pWhois.Administrative.Name)
			//fmt.Printf("Administrative.Organization: %s\n",pWhois.Administrative.Organization)
			//fmt.Printf("Administrative.Email: %s\n",pWhois.Administrative.Email)
			if strings.Contains(pWhois.Administrative.Name,pattern) {
				retval = true
			}
			if strings.Contains(pWhois.Administrative.Organization,pattern) && retval == false {
				retval = true
			}
			if strings.Contains(pWhois.Administrative.Email,pattern) && retval == false {
				retval = true
			}
		}
	}
	
	return retval
}

func dnsGetSoaServers(hostname string) []string {
	var soa []string
	retries := 2
	dnsClient := retryabledns.New(DefaultResolvers, retries)
	dnsResponse, _ := dnsClient.Query(hostname, dns.TypeSOA)
	s := strings.Split(dnsResponse.Raw,"SOA")
	if len(s) == 3 {
		s1 := s[2]
		s2 := strings.ReplaceAll(s1, "\t", "")    
		s3 := strings.ReplaceAll(s2, "\n", "")
		s4 := strings.ReplaceAll(s3, ". ", ":")
		s5 := strings.Split(s4, ":")
		for _,t := range s5 {
			if strings.Contains(t,".") {
				if strings.Contains(t,".") {
					if sliceContainsElement(RootSoa,t) == false {
						soa = append(soa,t)
					}
				}
			}
		}
	}
	return soa
}

func buildKnownHostsSoaDb(channel chan []string, knownDomainsList []string) {
	var knownSoaHosts []string
	fmt.Println("[*] Building SOA record database")
	
	for _,knownDomain := range knownDomainsList {
		soahosts := dnsGetSoaServers(knownDomain)
		for _,soa := range soahosts {
			knownSoaHosts = append(knownSoaHosts, soa)
		}
	}
	uniqueSoaHosts := sliceUniqueElements(knownSoaHosts)
	channel <- uniqueSoaHosts
	close(channel)
}

func dnsCheck(knownSoaHosts []string, host string) bool {
	retval := false
	targetSoa := dnsGetSoaServers(host)

	for _, soa := range targetSoa {
		if sliceContainsElement(knownSoaHosts,soa) {
			retval = true
		}
	}
	return retval
}

func hostVerify(ch chan []string, options *Options, host string, wg * sync.WaitGroup) {
	var match = false
	var knownSoaHosts []string
	defer wg.Done()

	if len(options.Domain) > 0 {
		dt := ParseDomainTokens(host)
		if dt.Domain == options.Domain {
			match = true
			fmt.Printf("SUB:%s\n",host)
		}
	} else if len(options.Domains) > 0 {
		for _,knownDomain := range options.Domains {
			dt := ParseDomainTokens(host)
			
			if dt.Domain == knownDomain {
				match = true
				fmt.Printf("SUB:%s\n",host)
			}
		}
	}
	
	if match == false && len(options.WhoisString) > 0 {
		match = whoisCheck(options.WhoisString, host)
		if match == true {
			fmt.Printf("WHOIS:%s\n",host)
		}
	}
	
	//buildKnownHostsSoaDb processing result:
	for msg := range ch {
		knownSoaHosts = msg
	}

	if len(knownSoaHosts) == 0 {
		if match == false && len(options.Domain) >0 {
			match = dnsCheck(knownSoaHosts, host)
			if match == true {
				fmt.Printf("SOA:%s\n",host)
			}
		} else if match == false && len(options.Domains) > 0 {
			match = dnsCheck(knownSoaHosts, host)
			if match == true {
				fmt.Printf("SOA:%s\n",host)
				
			}
				// vou ter que o soa de todos os known antecipadamente\
				// e so entao verificar se algum deles da match com o target domain
				// da forma que esta hoje ,eu checo todos com todos, e ta dando muito resultaod repitido 
			
		}
	}
	
}

func main() {	
	options := parseOptions()

	var wg sync.WaitGroup

	var host string
	var hosts []string
	var temp []string
	var hostname string
	
	ch := make(chan []string)
	
	if len(options.Host) > 0 {
		host = options.Host
		temp = append(temp, options.Domain)
		go buildKnownHostsSoaDb(ch, temp)
		go hostVerify(ch, options, host, &wg)

	} else {
		bytesRead, _ := ioutil.ReadFile(options.Hosts)
		file_content := string(bytesRead)
		hosts = strings.Split(file_content, "\n") // a slice with all hostnames
		
		bR, _ := ioutil.ReadFile(options.Domainf)
		fc := string(bR)
		options.Domains = strings.Split(fc, "\n")

		go buildKnownHostsSoaDb(ch, options.Domains)
		
		for _, hostname = range hosts {
			if len(hostname) > 2 {
				wg.Add(1)
				go hostVerify(ch,options, hostname, &wg)
			}
		}
	}

	

	wg.Wait()
}



